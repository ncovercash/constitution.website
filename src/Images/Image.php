...
use \WeTheFuture\Database\Query\MultiInsertQuery;
...
class Image {
	 ...
	 */
	public function __construct(string $folder, string $fileToken, ?string $path, string $caption="") {
		if ($path == "default.png") {
			$path = null; // BC
		}
	...
	public function getImgElementHtml(array $additionalClasses=[]) : string {
		...
		foreach ($paths as $path) {
			...
			if (Controller::isDevelMode() && file_exists($path[1])) { // file_exists because rel paths
				$str .= ' data-size="'.htmlspecialchars((string)filesize($path[1])).'"';
			}
			$str .= '>';
		}
	...
	public static function getNewItemImage() : self {
		return new self(Folders::PLACEHOLDERS, "", "new.png");
	}
	...
	public static function getLogoImage() : self {
		return new self(Folders::LOGO_WHITE, "", "logo.png");
	}
}

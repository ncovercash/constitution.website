...
namespace WeTheFuture\Images;

use \WeTheFuture\Database\Column;
use \WeTheFuture\Database\QueryAddition\WhereClause;
use \WeTheFuture\Database\Query\{DeleteQuery, MultiInsertQuery, UpdateQuery};
...
class DBImage extends Image {
	 ...
	 */
	public function __construct(int $id=0, int $parentId=0, array $dbInfo, string $folder, string $fileToken, ?string $path=null, string $caption="", string $captionInfo="", int $sort=0, bool $pendingInsertion=false) {
		$this->setId($id);
		...
		parent::__construct($folder, $fileToken, $path, trim($caption.($captionInfo ? ("\n".$dbInfo["captionDelimiter"].$captionInfo) : '')));
		if ($pendingInsertion) {

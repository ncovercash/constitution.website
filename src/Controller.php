...
use \WeTheFuture\Database\{AbstractDatabaseModel, AbstractDatabaseRowModel};
use \WeTheFuture\Email\Email;
use \WeTheFuture\HTTPCode;
use \WeTheFuture\Images\{DBImage, Image};
...
class Controller {
	...
	public static function loadClass(string $name) : void {
		if (strpos($name, 'WeTheFuture\\') === 0) {
			$unNamespaced = str_replace('WeTheFuture\\', '', $name);
	...
	public static function sendErrorNotice(string $subj, string $errco, int $errno, string $errstr, string $errfile, int $errline, string $trackingId) : void {
		if (!class_exists("\\WeTheFuture\\Secrets") || !class_exists("\\PHPMailer\\PHPMailer\\PHPMailer")) {
			return;
		}
		ob_start();
		$destinations = [];
	...
	public static function send500Error(int $errno, string $errstr, string $errfile, int $errline, string $trackingId) : void {
		...
		if (Endpoint::isEndpoint()) {
			...
			if (headers_sent()) { // assume we already sent header, despite our OB attempts
				...
				echo '<p class="flow-text no-top-margin">If you would please e-mail <a href="mailto:smileytechguy@smileytechguy.com">smileytechguy@smileytechguy.com</a> with the above tracking ID, what you were doing, and any other relevant information.</p>';
				...
				if (self::isDevelMode()) {
					...
					echo '<p>SECRETS DEFINED: '.(class_exists("\\WeTheFuture\\Secrets") ? "YES" : "NO").'</p>';
					$version = array_map("intval", explode(".", phpversion() ?: "0.0.0"));
					...
					echo '<p></p>';
					echo '<p>PHP Version: '.htmlspecialchars(phpversion() ?: "0.0.0").'</p>';
					echo '<p>WeTheFuture Version: '.htmlspecialchars(Controller::getVersion()." (".self::getCommit().")").'</p>';
					echo '</div>';
				}
	...
	public static function getCommit() : string {
		self::$commit = substr((string)microtime(true), -7);

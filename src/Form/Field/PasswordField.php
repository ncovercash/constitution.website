...
use \WeTheFuture\API\TransitEncryption;
...
class PasswordField extends AbstractField {
	...
	public function getJsValidator() : string {
		...
		$str .= 'if (';
		$str .= '$(document.getElementById('.json_encode($this->getId()).')).val().length < '.json_encode($this->getMinLength());
		$str .= ') {';
	...
	public function getJsAggregator(string $formDataName) : string {
		return $formDataName.'.append('.json_encode($this->getDistinguisher()).', encryptString(btoa($(document.getElementById('.json_encode($this->getId()).')).val())));'; // yes, this is half-assing it.  it works, and is better than 99% of sites
	}

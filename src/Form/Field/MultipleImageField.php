...
class MultipleImageField extends ImageField {
	...
	public function getJsAggregator(string $formDataName) : string {
		...
		$str .= 'window.log('.json_encode(basename(__CLASS__)).', '.json_encode($this->getId()." - aggregating ").'+$(document.getElementById('.json_encode($this->getId()).'))[0].files.length+'.json_encode(" images").');';

		$str .= 'for (var i=0; i<$(document.getElementById('.json_encode($this->getId()).'))[0].files.length; i++) {';
		$str .= $formDataName.'.append('.json_encode($this->getDistinguisher().'[]').', $(document.getElementById('.json_encode($this->getId()).'))[0].files[i]);';
		$str .= '}';

...
use \WeTheFuture\API\Response;
use \WeTheFuture\Form\Form;
use \WeTheFuture\Images\MIMEType;
use \WeTheFuture\HTTPCode;
use \WeTheFuture\Page\UniversalFunctions;
...
class ImageField extends AbstractField {
	...
	public function getJsValidator() : string {
		...
		if ($this->isRequired()) {
			$str .= 'if (';
			$str .= '$(document.getElementById('.json_encode($this->getId()).'))[0].files.length === 0';
			$str .= ') {';
		...
		$str .= 'for (var i=0;i<$(document.getElementById('.json_encode($this->getId()).'))[0].files.length;i++) {';

		$str .= 'if (';
		$str .= '$(document.getElementById('.json_encode($this->getId()).'))[0].files[i].size >= '.json_encode($this->getMaxSize());
		$str .= ') {';
		...
		$str .= 'if (';
		$str .= '!'.json_encode(MIMEType::getMimeTypes()).'.includes($(document.getElementById('.json_encode($this->getId()).'))[0].files[i].type)';
		$str .= ') {';
		$str .= 'window.log('.json_encode(basename(__CLASS__)).', '.json_encode($this->getId()." - file ").'+i+'.json_encode(" is not a supported type (").'+$(document.getElementById('.json_encode($this->getId()).'))[0].files[i].type+'.json_encode(")").', true);';
	...
	public function getJsAggregator(string $formDataName) : string {
		return $formDataName.'.append('.json_encode($this->getDistinguisher()).', $(document.getElementById('.json_encode($this->getId()).'))[0].files[0]);';
	}

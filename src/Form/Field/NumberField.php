...
class NumberField extends AbstractField {
	...
	public function getJsValidator() : string {
		...
		$str .= 'if (';
		$str .= '$(document.getElementById('.json_encode($this->getId()).')).val() < '.json_encode($this->getMin());
		$str .= ') {';
		...
		$str .= 'if (';
		$str .= '$(document.getElementById('.json_encode($this->getId()).')).val() > '.json_encode($this->getMax());
		$str .= ') {';
	...
	public function getJsAggregator(string $formDataName) : string {
		return $formDataName.'.append('.json_encode($this->getDistinguisher()).', Math.round(parseFloat($(document.getElementById('.json_encode($this->getId()).')).val()) * Math.pow(10, '.$this->getPrecision().')) / Math.pow(10, '.$this->getPrecision().'));';
	}

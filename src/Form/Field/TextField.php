...
class TextField extends AbstractField {
	...
	public function getJsValidator() : string {
		...
		if ($this->getMaxLength() > 0) {
			$str .= 'if (';
			$str .= '$(document.getElementById('.json_encode($this->getId()).')).val().length > '.json_encode($this->getMaxLength());
			$str .= ') {';
		...
		$str .= 'if (';
		$str .= json_encode($this->getDisallowed()).'.includes($(document.getElementById('.json_encode($this->getId()).')).val())';
		$str .= ') {';
